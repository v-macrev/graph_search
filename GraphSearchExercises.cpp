...
int GridGraph::countEdges() const {
    ...
    for (const auto &point:adjacencyMap) {
        numEdges += point.second.size(); // the size of this point's adjacent set
    }
    numEdges = numEdges / 2;
    return numEdges;
}
...
std::list<IntPair> graphBFS(const IntPair &start, const IntPair &goal, const GridGraph &graph) {
    ...
    if (tooManySteps) {
        std::cout
                << "graphBFS warning: Could not reach goal within the maximum allowed steps.\n (This may be expected if no path exists.)"
                << std::endl << std::endl;
        return std::list<IntPair>();
    }
...
std::list<PuzzleState> puzzleBFS(const PuzzleState &start, const PuzzleState &goal) {
    ...
    if (tooManySteps) {
        std::cout
                << "puzzleBFS warning: Could not reach goal within the maximum allowed steps.\n (This may be expected if no path exists.)"
                << std::endl << std::endl;
        return std::list<PuzzleState>();
    }
