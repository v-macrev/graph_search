...
int GridGraph::countEdges() const {
  ...
  // Counting edges by iterating through each neighbor set
  for (const auto& pair : adjacencyMap) {
    // Each neighbor set represents edges connected to a point
    // We count half of the size to avoid double counting
    numEdges += pair.second.size() / 2;
  }
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  // Removing references to p1 from its neighbors' neighbor sets
  ...
  // =======================================================================
  // Erase the point key itself from adjacencyMap
...
std::list<IntPair> graphBFS(const IntPair& start, const IntPair& goal, const GridGraph& graph) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    GridGraph::NeighborSet neighbors = graph.adjacencyMap[curPoint];
    // =====================================================================
...
std::list<PuzzleState> puzzleBFS(const PuzzleState& start, const PuzzleState& goal) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    auto neighbors = curState.getNeighbors(); // Use the PuzzleState function to get neighbors.
    ...
    for (auto neighbor : neighbors) {
      ...
      // TODO: Your code here!
      // Check whether the neighbor has already been visited.
      bool neighborWasAlreadyVisited = visitedSet.count(neighbor); // Check if neighbor has been visited.
